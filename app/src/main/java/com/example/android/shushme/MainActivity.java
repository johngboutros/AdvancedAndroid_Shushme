...
public class MainActivity extends AppCompatActivity implements
    ...
    private void refreshPlacesData() {
        ...
        if (cursor == null || cursor.getCount() == 0) {
            Log.i(TAG, "refreshPlacesData: nothing returned. aborting...");
            return;
        }
        ...
        while (cursor.moveToNext()) {
            String placeId = cursor.getString(cursor.getColumnIndex(
                    PlaceContract.PlaceEntry.COLUMN_PLACE_ID));
            placeIds.add(placeId);
        }

        PendingResult<PlaceBuffer> placesResult = Places.GeoDataApi
                .getPlaceById(mClient, placeIds.toArray(new String[placeIds.size()]));


        placesResult.setResultCallback(new ResultCallbacks<PlaceBuffer>() {
            @Override
