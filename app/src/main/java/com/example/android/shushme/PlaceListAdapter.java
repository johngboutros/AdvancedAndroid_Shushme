...
public class PlaceListAdapter extends RecyclerView.Adapter<PlaceListAdapter.PlaceViewHolder> {
    ...
    public PlaceListAdapter(Context context, PlaceBuffer placeBuffer) {
        // DONE (4) Take a PlaceBuffer as an input and store it as a local private member mPlaces
    ...
    public void onBindViewHolder(PlaceViewHolder holder, int position) {
        // DONE (6) Implement onBindViewHolder to set the view holder's Name and Address text fields
        // from the Place object at the specified position in mPlaces
        if (mPlaces != null && mPlaces.getCount() > position) {
            Place place = mPlaces.get(position);

            TextView nameView = holder.itemView.findViewById(R.id.name_text_view);
            TextView addressView = holder.itemView.findViewById(R.id.address_text_view);
    ...
    //DONE (7) Implement a public method swapPlaces that replaces the current mPlaces PlaceBuffer with a new one
    public PlaceBuffer swapPlaces(PlaceBuffer places) {
        PlaceBuffer temp = null;
        if (mPlaces != null) {
            temp = mPlaces;
        }
        mPlaces = places;
